using MovieLibrary.Application.Utils.Structures;
...
public class CreateUserCommandValidator : AbstractValidator<CreateUserCommand>
{
    ...
    public CreateUserCommandValidator(IUserRepository userRepository)
    {
        RuleFor(command => command.Username)
            ...
            .MustAsync(async (username, _) => !await userRepository.ExistsAsync(user => user.Username.Equals(username)))
        ...
        RuleFor(command => command.Email)
            ...
            .Must(email => new EmailAddress(email).IsValid())
            .WithMessage("The e-mail address is not valid.")
            .MustAsync(async (email, _) => !await userRepository.ExistsAsync(user => user.Email.Equals(email)))
